<template>
     <Form @submit="submitContact" :validation-schema="contactFormSchema">
         ...
         <div class="form-group">
             ...
             <button v-if="showDelete || canDelete" type="button" class="ml-2 btn btn-danger" @click="deleteContact">
                 Xóa
             </button>
 ...
 export default {
     ...
     data() {
         ...
         return {
             // clone from prop; no fake id
             contactLocal: this.contact ? { ...this.contact } : {},
             contactFormSchema,
         };
     },
     computed: {
         // show Delete if backend provides _id or id
         canDelete() {
             const c = this.contactLocal || {};
             return !!(c._id || c.id);
         }
     },
     methods: {
     ...
     watch: {
         // keep local state in sync when the async contact prop arrives/changes
         contact: {
             deep: true,
             immediate: true,
             handler(newVal) {
                 this.contactLocal = newVal ? { ...newVal } : {};
             }
         }
     }
 };
